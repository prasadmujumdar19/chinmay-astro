...
| WF | Name | Registry Status | Priority | n8n Actual | Notes |
|---|---|---|---|---|---|
| WF-10 | Slack Admin Handler | 🟢 Active | 🔴 P1 | n8n: "WF-10 Slack Admin Handler" (wMh0oBRtJbvhLgOf) | Entry point for Slack events. Filters bot messages. Routes commands to WF-11. Admin text in user channels relays to WF-41 only if user is consultation_active (TD-023 May 2026 — status guard added). Updated 2026-05-18 (BUG-01): "Load User Status" `options.queryReplacement` converted from comma-separated expression string to JS-array form `={{ [a, b, c] }}` — admin messages containing commas were being split by n8n's comma-parser and misaligning the `$1/$2/$3` placeholders. 2026-05-20 TD-003 F2: added `Build WF-60 Payload (Slack Inbound)` Code mapper + `Call WF-60 Message Logger` executeWorkflow (tv 1.2, onError=continueRegularOutput) as a 3rd parallel side branch from `Extract Required Fields`. Logs every Slack inbound that has a user_channel match; admin-wide commands (LIST/STATS/HELP) get silently dropped by WF-60's skip-if-not-resolvable path. |
