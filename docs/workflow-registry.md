...
| WF-42 | Consultation Closer | 🟢 Active | 🔴 P1 | n8n: "WF-42 Consultation Closer" (fx70vqyJtRdF2DgR) | Admin issues CLOSE in consult channel. State guard added (TD-022 May 2026): only proceeds if user is consultation_active. Sends interactive button message with btn_feedback / btn_rebook (TD-015 May 2026 — replaces unapproved template). Updates status=consultation_closed. **Does NOT archive Slack channel** — channel intentionally kept open for reuse by WF-45 on rebook (TC-0305 design decision 2026-05-16). |
