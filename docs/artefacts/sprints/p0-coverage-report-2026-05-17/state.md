...
followup_execution:
  ...
  user_triaged: "2026-05-17 — User reviewed 11 new findings + 4 FU-1 carry-overs. Tagged 4 items (FU-6 through FU-9) as PENDING for next sprint; rest (passthrough mapping, __rl regression behavior) explicitly skipped as pure conformance."
  pending_followups_next_sprint:
    - id: FU-6
      summary: "WF-46 admin-notification messageText field-name fix ($json.blockReason → $('When Executed by Another Workflow').item.json.reason; same for adminUserId)"
      change_type: Surgical
    - id: FU-7
      summary: "Add IF empty-result guards after WF-32, WF-31, WF-45 Postgres SELECT lookups. User has manually set alwaysOutputData=true on all 3; verify live before starting"
      change_type: Structural (3 WFs)
    - id: FU-8
      summary: "Bump executeWorkflow typeVersion 1 → 1.1 across WF-20 (2 nodes), WF-45 (1 node), WF-12 (1 node). Batch Surgical Mode C"
      change_type: Surgical-Batch
    - id: FU-9
      summary: "WF-42 admin Slack posts refactored to WF-51 (mirrors FU-1 WF-46 pattern). Likely Mode A regenerate-by-copy"
