...
| **WF-30 / WF-40** | ✅ correct | Both map `messageContent` explicitly → WF-25 classifies real text. |
...
**Net confirmed HIGH (sprint-group):** WF-31 + WF-43 pass the wrong key (`messageText`) to the WF-25 classifier → both misclassify free-form text. Single-line fix each (`messageText` → `messageContent`, sourced from `$json.messageContent`), but a live workflow edit — held pending direction (guidance point 1). The WF-25 entry-guard gap is the structural root cause worth addressing alongside.

**Severity downgrade:** the 4 MED "empty defineBelow" items in PART A/B are withdrawn (false positives).
