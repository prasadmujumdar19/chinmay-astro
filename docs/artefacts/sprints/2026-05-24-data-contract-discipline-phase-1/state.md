...
wave_2_apply_progress:
  ...
  - sub_id: sub-12
    status: done
    target_workflows: [WF-20, WF-40, WF-43, WF-44]
    applied_at: 2026-05-24T18:35:46Z
    n8n_edits: "WF-20: Send HELP Response value rewritten (messageBody→messageContent + messageType:'text', 5 ternary user.status envelope reads); Normalize Keyword userId $json.user.id. WF-40 STRICT: Load User Record removed, trigger→Call WF-25 direct rewire, Call WF-25 + Format Slack Message rewritten to envelope paths. WF-43: Extract Gemini Reply jsCode rewrite — final return uses messageType+messageContent. WF-44: Prepare Ack Message jsCode + Send Ack via WF-50 workflowInputs both canonical (messageType+messageContent)."
    pseudo_updated: [WF-20, WF-40, WF-43, WF-44]
    verify: "All 4 post-PUT greps green: WF-20 messageBody absent + envelope user.status reads present (no flat json.userStatus); WF-40 Load User Record node + connections absent + trigger→Call WF-25 direct; WF-43 'message: reply' absent + messageContent: reply present; WF-44 Prepare Ack + Send Ack key set [phoneNumber,messageType,messageContent] confirmed."
    items_advanced: [TD-DCP-052 (sub-12 portion), TD-DCP-040 (sub-12 portion), TD-DCP-030 (sub-12 portion)]
    strict_findings_resolved: ["WF-40 'Load User Record' redundant SELECT", "WF-43 legacy 'message' key", "WF-44 legacy 'message' key"]
  - sub_id: sub-13
    status: done
    target_workflows: [WF-32, WF-33, WF-42, WF-46]
    applied_at: 2026-05-24T18:41:06Z
    n8n_edits: "All 4 WFs: Load-User SELECT removed + connection rewired. WF-32: Prepare Admin Notification jsCode → envelope; trigger fan-out Call WF-50→Prepare Admin Notification direct. WF-33: 3 queryReplacement updates (Update Payment Status, Update User Status, Create Consultation Record) to envelope user.id; Prepare User Activation Message + Prepare WF-51 Payload (Notify Admin) jsCode rewrites; Extract Command Data→Update Payment Status direct rewire. WF-42: 2 queryReplacement updates (Close Consultation Record, Update User Status) + Prepare Feedback Message + Prepare WF-51 Payload (Notify Admin Closed) jsCode rewrites; trigger→Close Consultation Record direct rewire. WF-46: Update User to Blocked Status queryReplacement + Prepare WF-51 Payload (Notify Admin Blocked) jsCode rewrite; trigger→Update User to Blocked Status direct rewire."
    pseudo_updated: [WF-32, WF-33, WF-42, WF-46]
    pseudo_renumber: "WF-33 and WF-42 had tombstone steps in prior pseudo; rewritten linearly per [[feedback_pseudo_linear_numbering]]"
    verify: "All 4 post-PUT verifications green: Load-User node absent + envelope-path expressions in queryReplacements / jsCode + trigger connection points to first remaining downstream node."
    items_advanced: [TD-DCP-052 (sub-13 portion), TD-DCP-062 (sub-13 portion)]
    strict_findings_resolved: ["WF-32 Load User Channel from DB", "WF-33 Load User by Phone", "WF-42 Load User by Phone", "WF-46 Load User by Phone"]
  - sub_id: sub-14
    status: done
    target_workflows: [WF-41, WF-34]
    applied_at: 2026-05-24T18:43:42Z
    n8n_edits: "WF-41 STRICT (latent runtime bug fix): Prepare WhatsApp Message jsCode rewrite — read input.messageText (was legacy input.adminMessage). Without this fix, every admin relay would emit messageContent=undefined and fail WF-50 entry guard. WF-34: Load User by Phone query simplified to fetch ONLY payment_id (JOIN against users for phone lookup); Prepare Rejection Message + Prepare WF-51 Payload (Notify Admin Rejected) jsCode rewritten to read user fields from trigger envelope ($('When Executed by Another Workflow').first().json.user.*). No node removal in WF-34 — node retained for payment_id."
    pseudo_updated: [WF-41, WF-34]
    pseudo_renumber: "WF-34 prior pseudo had tombstone steps 3 & 4; rewritten linearly per [[feedback_pseudo_linear_numbering]]"
    verify: "Post-PUT greps green: WF-41 messageText present + adminMessage absent; WF-34 simplified query landed + Prepare Rejection Message uses trigger.user.phone_number + Notify Admin uses trigger.user; no Load User by Phone references remain in any jsCode."
    items_advanced: [TD-DCP-062 (sub-14 portion), TD-DCP-040 (sub-14 portion)]
    strict_findings_resolved: ["WF-41 latent runtime bug — adminMessage→messageText field name mismatch with Wave 1 Relay Envelope"]
